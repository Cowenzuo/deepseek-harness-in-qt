...
QList<CheckItem> probeDependencies(const QString &sourcePath)
{
...

    {
        ...
        it.passed = QDir(sourcePath + QStringLiteral("/node_modules")).exists();
        it.detail =
...

    {
        ...
        it.passed = QDir(sourcePath + QStringLiteral("/apps/web/dist")).exists();
        it.detail =
    ...
    // 库构建产物（packages 下任一非空 lib）
    {
        ...
        bool found = false;
        QDirIterator itr(sourcePath + QStringLiteral("/packages"),
